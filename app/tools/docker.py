...
class DockerTool:
    ...
    def is_command_allowed(self, command: list[str]) -> bool:
        return len(command) >= 2 and command[0] == "docker" and command[1] in self._ALLOWED

    def validate_input(self, request: ToolRequest) -> None:
        if not self.is_command_allowed(request.command):
            raise PermissionError(f"docker command '{' '.join(request.command)}' is not allowlisted")
