...
class TerraformPlanTool:
    tool_name = "terraform"
    _ALLOWED = {"fmt", "validate", "plan"}

    def __init__(self, *, workspace: str | Path, timeout_seconds: float = 120) -> None:
    ...
    def is_command_allowed(self, command: list[str]) -> bool:
        return len(command) >= 2 and command[0] == "terraform" and command[1] in self._ALLOWED

    def validate_input(self, request: ToolRequest) -> None:
        if not self.is_command_allowed(request.command):
            raise PermissionError(f"terraform command '{' '.join(request.command)}' is not allowlisted")
