...
async def test_openai_provider_success_path_parses_json_and_sends_request():
    ...
    assert result == {"thinking_summary_delta": "parsed", "answer": {"ok": True}}
...
@pytest.mark.asyncio
async def test_openai_provider_rejects_non_json_without_leaking_api_key():
    async def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            200,
